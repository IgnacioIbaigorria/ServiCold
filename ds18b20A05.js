...
function getThreshold() {
    ...
    return fetch('https://servicoldingenieria.com/umbralDSA02.php', { cache: 'no-cache' })
...
function checkTemp() {
    ...
    fetch('https://servicoldingenieria.com/checkTempA02.php',  {cache: 'no-cache' })
        .then(response => response.json())
        .then(data => {
...
function sendEmailAlert() {
    ...
    fetch('https://servicoldingenieria.com/back-end/emailDSA02.php')
    .then(response => response.json())
    .then(data => {
        if (data.length > 0) {
...
function loadAllData(selectedDate) {
    ...
    let url = `https://servicoldingenieria.com/DSGetA02.php?data_limit=${dataLimit}`;
