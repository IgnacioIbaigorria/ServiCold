...
if ($con) {
    ...
    if ($selectedDate) {
        ...
        $consulta = "SELECT nivel_combustible, fecha_actual FROM `FL902A12` WHERE DATE(fecha_actual) = '$selectedDate' ORDER BY fecha_actual DESC";
    } else {
        // Consulta original sin filtro por fecha
        $consulta = "SELECT nivel_combustible, fecha_actual FROM `FL902A12` ORDER BY fecha_actual DESC";
    }
