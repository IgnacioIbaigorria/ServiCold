...
function getThreshold() {
    ...
    return fetch('https://servicoldingenieria.com/back-end/umbral/umbralDS(1).php', { cache: 'no-cache' })
...
function checkTemp() {
    ...
    fetch('https://servicoldingenieria.com/back-end/check-Temp_Comb/checkTemp(1).php',  {cache: 'no-cache' })
        .then(response => response.json())
        .then(data => {
...
function sendEmailAlert() {
    ...
    fetch('https://servicoldingenieria.com/back-end/emailDS(1).php')
    .then(response => response.json())
    .then(data => {
        if (data.length > 0) {
...
function loadAllData(selectedDate) {
    ...
    let url = `https://servicoldingenieria.com/back-end/GET/DSGet1.php?data_limit=${dataLimit}`;
...
function getThreshold() {
    console.log("Se está ejecutando la función getThreshold()");
    return fetch('https://servicoldingenieria.com/umbralDS(1).php', { cache: 'no-cache' })
        .then(response => response.json())
        .then(data => {
            if (data && typeof data.umbral !== 'undefined') {
                temperatureThreshold = data.umbral;
...
function checkTemp() {
    console.log("Se está ejecutando la función checkTemp()");
    // Realiza la solicitud HTTP GET para obtener los datos del sensor de nivel de combustible
    fetch('https://servicoldingenieria.com/checkTemp(1).php',  {cache: 'no-cache' })
        .then(response => response.json())
        .then(data => {
            // Verifica si hay datos
            if (Array.isArray(data) && data.length > 0) {
...
function loadAllData(selectedDate) {
    // URL para la solicitud a tu script PHP
    let url = `https://servicoldingenieria.com/DSGet1.php?data_limit=${dataLimit}`;
    // Agrega el parámetro `selected_date` a la URL si se proporciona una fecha seleccionada
    if (selectedDate) {
        url += `&selected_date=${selectedDate}`;
    }
