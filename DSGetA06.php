...
if ($con) {
    ...
    if ($selectedDate) {
        ...
        $consulta = "SELECT Temperatura, fecha_actual FROM `ds18b20A06` WHERE DATE(fecha_actual) = '$selectedDate' ORDER BY fecha_actual DESC";
    } else {
        // Consulta original sin filtro por fecha
        $consulta = "SELECT Temperatura, fecha_actual FROM `ds18b20A06` ORDER BY fecha_actual DESC";
    }
