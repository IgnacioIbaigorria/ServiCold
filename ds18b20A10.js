...
function getThreshold() {
    ...
    return fetch('https://servicoldingenieria.com/umbralDSA10.php', { cache: 'no-cache' })
...
function checkTemp() {
    ...
    fetch('https://servicoldingenieria.com/checkTempA10.php',  {cache: 'no-cache' })
        .then(response => response.json())
        .then(data => {
...
function sendEmailAlert() {
    ...
    fetch('https://servicoldingenieria.com/back-end/emailDSA10.php')
    .then(response => response.json())
    .then(data => {
        if (data.length > 0) {
...
function loadAllData(selectedDate) {
    ...
    let url = `https://servicoldingenieria.com/DSGetA10.php?data_limit=${dataLimit}`;
